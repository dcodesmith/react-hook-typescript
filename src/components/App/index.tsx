...
const App: React.FC = () => {
  const [{ data, error }, dispatch] = useReducer(medalReducer, INITIAL_STATE)
  ...
  const onEdit = (values: any): void => dispatch({ type: 'EDIT', item: values })

  const onDelete = (country: string): void => dispatch({ type: 'REMOVE', country })
  if (error) {
    alert(error)
  }
  return (
